...
class NeuralAgent implements IAgent {
    ...
    NeuralAgent(double[] weights) {
        tmpWeights = Arrays.copyOf(weights, weights.length);
        in = new double[20];
        nn = new NeuralNet();
    ...
    public double[] getTmpWeights() {
        return tmpWeights;
    }

    public void setTmpWeights(double[] tmpWeights) {
        this.tmpWeights = tmpWeights;
    }
    ...
    void setWeights2(double[] bigWeights) {
        double[] weights = new double[291];
        for (int i = 0; i < 291; i++)
            weights[i] = bigWeights[i];
        setWeights(weights);
    }
