...
class SabadoVladimir implements IAgent {
    protected int iter;
    ...
    protected int[] pathSavePoint = new int[NUMBER_OF_SPRITES];
    ...
    public SabadoVladimir() {
        reset();
    }

    public SabadoVladimir(float zoning_modifier) {
        this();
    ...
    public void resetSavePoints(){
        pathSavePoint = new int[NUMBER_OF_SPRITES];
...
//        }
    ...
    public void moveSpritePregen(int index, Model m, List<GameState> paths) {
        ...
        //If pathSavePoint is -1, then it is currently not following the path
        if (pathSavePoint[index] < 0 || pathSavePoint[index] >= paths.size()) {
            //Find the nearest path point
            pathSavePoint[index] = getNearestPathPoint(m, index, paths);
        }
        ...
        if (getDistance(spriteLocation, endLocation) <= 10f) {
            ...
            pathSavePoint[index] = paths.size() - 1; //Set save point to last location
            return;
        }
        ...
        //TODO: Bugged when path curves. Cannot assume path has increasing X values
...
//////                    System.out.printf("Going to %f,%f\n", currentGamestate.getLocation().getX(), currentGamestate.getLocation().getY());
...
//////        }
        if(getDistance(spriteLocation, paths.get(pathSavePoint[index]).getLocation()) < 10f){
            pathSavePoint[index]++;
        }
        moveSprites(index, m , paths.get(pathSavePoint[index]).getLocation(), false);
    ...
    /**
     * Get nearest point from the list of points in the path
     * @param m Model
     * @param index sprite index
     * @param paths List of points in the path
     * @return returns the index of the nearest path
     */
    public int getNearestPathPoint(Model m, int index, List<GameState> paths){
        if(paths == null || paths.isEmpty()){
            return -1;
        }

        Location spriteLocation = Location.getLocation(m, index);

        float shortestDistance = Float.MAX_VALUE;
        int nearestPoint = -1;
        for(int i = 0; i < paths.size(); i++){
            float tmpDistance = (float) getDistance( spriteLocation, paths.get(i).getLocation());
...
//                moveSpriteFollow(m, i, (List<GameState>) bestPathsToFlag[1]); //Everyone uses the middle
            }
    ...
    static class Astar {
        ...
        public GameState findPath(Location start, Location end) {
            ...
            while (!frontier.isEmpty()) {
                ...
                if (SabadoVladimir.getDistance(front.getLocation(), end) < getCloseEnoughRange()) {
                    return front;
                }
        ...
        public void generateChild(GameState current, String action, Location end) {
            ...
            child.heuristic = (float) SabadoVladimir.getDistance(child.getLocation(), end) / maxSpeed;
