...
class MyAIV3 implements IAgent {
    protected int iter;
    ...
    protected float energyThreshold = 0.6f;
    ...
    protected List<GameState> bestPathList = new ArrayList<>();
    ...
    public MyAIV3(float zoning_modifier) {
        this();
    ...
    public void update(Model m) {
        ...
        //Set up time balance
//        if (timeBalanceManager == null) {
//            timeBalanceManager = new TimeBalanceManager(m);
//        }
        //Set up Astar
        if (astar == null) {
            astar = new Astar(m);
        ...
        //Long startTime = System.nanoTime();
//        if (iter == 0) {
        //We don't really care if they're just going up and down, so Y is not necessary
//            prevX = m.getXOpponent(0);
//            for(int i = 0; i < NUMBER_OF_SPRITES; i++){
//                groupUp(m, i);
//            }
...
//        }
        ...
        if (getAliveOpponent(m) != 0) {
            //if (m.getXOpponent(0) != prevX || iter > GIVE_UP_MODIFIER) {
            if (engaged) {
                //System.out.println("Nearest Enemy:" + res.getLocationB());
                timeToFight(m, res);
            ...
            for (int i = 0; i < NUMBER_OF_SPRITES; i++) {
                ...
                //System.out.println("SpriteCount: " + m.getSpriteCountSelf());
    ...
    public void healMode(Model m, int index) {
        //Don't do in heal mode if alone, everyone is heading to heal. One must stay. Or if healed
        if (getAliveSprites(m) == 1 || isAllInHealMode() || m.getEnergySelf(index) > 0.9f) {
            healMode[index] = false;
        } else {
            //Check if too close to opponent
            Location myLocation = Location.getLocation(m, index);
            Result res = findNearestOpponent(m, myLocation);
            float safeZone = Model.MAX_THROW_RADIUS + Model.BLAST_RADIUS + 70;
            if (getSquareDistance(myLocation, res.getLocationB()) < Math.pow(safeZone, 2)) {
                //Find best retreat location that is farthest
                retreatOptimal(m, index, 5, 10, res.locationB);
            } else {
    ...
    public void timeToFight(Model m, Result res) {
        ...
        for (int i = 0; i < NUMBER_OF_SPRITES; i++) {
            ...
            //This calculation uses the sqrt, hence we do not square the other side
                ...
                if (m.getEnergySelf(i) < energyThreshold) {
                    retreatAndHeal(m, i, enemySquareDistance);
                    healMode[i] = true;
                    //System.out.println("Healing");
                } else {
                    //System.out.println("Move Sprites");
                    moveSprites(i, m, targetLocation);
                }
                avoidBombs(m, i);
            } else {
                ...
                if (bestPathsCurrentEnemy[i] == null || bestPaths.isEmpty()) {
                    ...
                    if (bestPathsCurrentEnemy[i] != null && currentEnemy[i] != null
                        ...
                        //System.out.println("Current Enemy:" + currentEnemy[i]);
    ...
    //Move
    //============================================
    public List<GameState> moveSprites(int i, Model m, Location destination) {
        ...
        return (List<GameState>) bestPathsCurrentEnemy[i];
    }
        ...
        for (int i = 0; i < paths.size(); i++) {
            ...
            if (spriteLocation.getX() < currentGamestate.getLocation().getX()) {
                ...
                if (getDistance(spriteLocation, currentGamestate.getLocation()) <= pregenThresholdDistance) {
                    ...
                    moveSprites(index, m, currentGamestate.getLocation());
                }
                return;
            }
...
//        System.out.println("Moving to: " + nextPath);
        m.setDestination(index, nextPath.getX(), nextPath.getY());
...
//        }
    ...
    private void moveSpriteFollowExtend(Model m, int index, Location newTarget) {
        ...
        if (isNewPathFurther) {
            ...
            if (!bestPaths.isEmpty()) {
                //This is the case if the new target is too far, but we still have some close ones on the list
                ...
                bestPaths = moveSprites(index, m, newTarget);
            }
        }
    ...
    public Result findNearestAliveOpponentToLocation(Model m, Location location) { //Pass my flag's x and y here
        ...
        for (int i = 0; i < NUMBER_OF_SPRITES; i++) {
            ...
            //TODO: This used to be the camping code. But is it not working and it might me messing up with targetting when near the enemy flag.
//            if (m.getEnergyOpponent(i) < 0) {
//                //continue; //ignore the dead opponents
//                //Select a dead target near their flag
//                if (getSquareDistance(opponentX, enemyFlag) <= MAX_THROW_SQUARE) {
//                    targetIndex = i;
//                    targetLoc = opponentLocation;
//                } else
//                    continue;
//            }
...
//            float d = sq_dist(x, y, m.getXOpponent(i), m.getYOpponent(i));
    ...
    private void spawnCamp(Model m, int index) {
    ...
    public void shootAtTarget(Model m, int i, Location targetLocation, boolean isFlag) {
        ...
        if (isFlag) {
            ...
            if (getSquareDistance(targetLocation, myLocation) <= MAX_THROW_SQUARE) {
                //If the current sprite is within the blast, just move out and dont even bomb
                //TODO: Camper code is not working
//                if (getDistance(myLocation, enemyFlag) < (Model.BLAST_RADIUS + 10) * (Model.BLAST_RADIUS + 10)) {
//                    moveSpritePregen(i, m);
//                    //ATTACK!!
//                    m.throwBomb(i, targetLoc.getX(), targetLoc.getY());
//                } else {
                //ATTACK!!
                m.throwBomb(i, targetLocation.getX(), targetLocation.getY());
                //avoidBombsOrHeal(m, i, true);
                attackCounters[i]--;
            } else {
                //Move closer to enemy flag
                moveSpritePregen(i, m, (List<GameState>) bestPathsToFlag[i]);
            }
...
//            float dx = m.getX(i) - m.getBombTargetX(nearestBoomTarget); //Maybe needs to be more dynamic
//            float dy = m.getY(i) - m.getBombTargetY(nearestBoomTarget);
//            if (dx == 0 && dy == 0)
//                dx = 1.0f;
...
//    }
    ...
    public void avoidBombs(Model m, int i) {
        ...
        //Location bombTarget = new Location( m.getBombTargetX(res.getIndex()), m.getBombTargetY()
    ...
    public void avoidOptimal(Model model, float offset, int simTime) {
        float[] bestX = new float[NUMBER_OF_SPRITES];
        float[] bestY = new float[NUMBER_OF_SPRITES];
        float[] health = new float[NUMBER_OF_SPRITES];
        for (int sim = 0; sim < 8; sim++) { // try 8 candidate destinations
            float[] xValues = new float[NUMBER_OF_SPRITES];
            float[] yValues = new float[NUMBER_OF_SPRITES];
            for (int i = 0; i < NUMBER_OF_SPRITES; i++) {
            ...
            for (int i = 0; i < NUMBER_OF_SPRITES; i++) {
                if (modelFork.getEnergySelf(i) > health[i]) {
                    bestX[i] = modelFork.getX(i);
                    bestY[i] = modelFork.getY(i);
                }
            }
    ...
    static class Astar {
        ...
        public GameState findPath(Location start, Location end) {
            ...
            while (!frontier.isEmpty()) {
                ...
                if (MyAIV3.getDistance(front.getLocation(), end) < getCloseEnoughRange()) {
                    return front;
                }
...
//                child.heuristic = (float)getDistance(current.getLocation(), end) * maxSpeed;
            ...
            child.heuristic = (float) MyAIV3.getDistance(child.getLocation(), end) / maxSpeed;
...
//======================================================================================================================
    ...
    static class Bomb {
        ...
        //Not sure if it is possible to get the start location
        Bomb(float xEnd, float yEnd) {
            this.xEnd = xEnd;
            this.yEnd = yEnd;
        }

        public boolean doesHit(float x, float y) {
            return (x - xEnd) * (x - xEnd) + (y - yEnd) * (y - yEnd) < Model.BLAST_RADIUS * Model.BLAST_RADIUS;
        }
